...
> **注**: 以下のエージェント/コマンド/スキルの列挙は **代表例** であり網羅性は保証しない。完全な一覧は対応ディレクトリ (`config/agents/{definitions,commands,skills,rules}/`, `config/genie/skills/`) と `modules/agent-skills.nix` 内の `agentDefinitions` / `agentCommands` リスト、および agent-skills-nix が拾う `enabledSkills` を参照する。ドキュメントは設計意図の説明、source of truth はコード側。
