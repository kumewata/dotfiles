...
- `config/agents/skills/` - Claude Code / OpenAI Codex 共通のスキル定義。agent-skills-nix 経由でデプロイ。Lakeview ダッシュボード設計者用の `steering-lakeview-handoff`、相互レビュー用の `codex-delegate` / `cc-delegate` 等を含む。
