...
4. **pivot**: 小計行 (自動小計) **非対応** (セル内条件付き書式は spec v4 で **対応済み**)
...
- **セル内条件付き書式は対応済み (spec v4)**: `encodings.cells.fields[].style.rules` で `startswith` 等のテキスト関数条件によるセル色付けが可能。table / pivot 両対応 (公式ドキュメント「Table and pivot table visualizations」に明記、2026-07 実機確認)。※以前「非対応」と記載していたが誤り
