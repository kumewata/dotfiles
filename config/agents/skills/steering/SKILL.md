...
1. **リポジトリ名の自動検出**
   ...
   **ディレクトリ名への変換**: `owner/repo` のスラッシュをハイフンに変換する。
...
4. **テンプレートからファイル作成**
   ...
   - `~/.claude/skills/steering/templates/requirements.md` → `~/.local/state/steering/<owner>-<repo>/YYYYMMDD-[機能名]/requirements.md`
   - `~/.claude/skills/steering/templates/design.md` → `~/.local/state/steering/<owner>-<repo>/YYYYMMDD-[機能名]/design.md`
   - `~/.claude/skills/steering/templates/tasklist.md` → `~/.local/state/steering/<owner>-<repo>/YYYYMMDD-[機能名]/tasklist.md`

5. **YAML Front Matter の記入**
   ...
   - **repos**: リポジトリ名自動検出で取得した `owner/repo` をリスト形式で設定。横断タスクの場合はユーザーが手動で追加可能
   - **branch**: `git rev-parse --abbrev-ref HEAD` で現在のブランチ名を取得して設定。detached HEAD の場合（出力が `HEAD`）は `branch` フィールドを省略する
...
リポジトリ内に `.steering/` ディレクトリが存在する場合、中央ディレクトリに移行する:

1. リポジトリ名自動検出（手順1と同じロジック）で `owner/repo` を取得
2. `.steering/` 配下の各プロジェクトを `~/.local/state/steering/<owner>-<repo>/` に移動
3. 移行したファイルの YAML Front Matter に `repos` / `branch` フィールドを追加
4. 移行元の `.steering/` ディレクトリを削除

**注意**: `.steering/` は `.gitignore` 管理のため、git 履歴への影響はない。
...
現在のリポジトリに関連するタスクを中央ディレクトリから発見する:

1. **リポジトリ名を自動検出**（モード1の手順1と同じロジック）
2. **直接所属タスクを検索**:
   ```
   ~/.local/state/steering/<owner>-<repo>/ 配下の tasklist.md を走査
   ```
3. **横断タスクを検索**（必要な場合）:
   ```
   ~/.local/state/steering/ 配下の全 tasklist.md の repos フィールドを検索し、
   現在のリポ名を含むタスクを発見
   ```
4. **アクティブなタスクをフィルタリング**:
   - YAML Front Matter の `status` が `pending` または `in_progress` のタスクのみ表示
   - `status` フィールドは tasklist.md 専用
