...
description: |
  Use when writing or revising Japanese technical prose — GitHub PR descriptions, docs / README, design notes, articles or blog posts, and commit messages written in Japanese — and you want the text to follow objective Japanese technical-writing norms (一文一行, 中黒・ダッシュの扱い, パラグラフライティング, 論証の厳密さ, LLM っぽい空句の禁止, 冗長の排除, 見出しの付け方). Complements the `tone` skill: this skill applies objective writing norms, while `tone` reproduces the user's personal voice — use both together (norms first, then voice). Do NOT trigger for English-only output, or for code-only changes with no Japanese prose. Trigger especially when the user says 日本語で PR 本文 / docs / 記事 / コミットメッセージ を書いて・推敲して, 文章規範, 一文一行, 中黒をやめて, LLM っぽい, 冗長を削って, or 日本語の技術文書を整えて.
---

# 日本語技術文書の文章規範 skill

日本語で技術的な文書（PR 本文、docs、README、設計メモ、記事、解説文、コミットメッセージ）を書く・推敲するときに、客観的な文章規範を適用する。

## tone skill との棲み分け

この skill は**客観的な文章規範**（一文一行、中黒・ダッシュの扱い、パラグラフライティング、LLM 口調の禁止など）を適用する。個人の文体を再現する `tone` skill とは責務が異なり、競合しない。規範に沿った素文をこの skill で作り、そのうえで `tone` で本人の文体に寄せる想定とする（規範が先、文体が後）。

## 出典

本規範は k16shikano 氏の gist「日本語技術文書の文章規範」を転記したもの。gist が消えても skill 単体で機能するよう全文を自己完結させている。原典の更新には手動で追従する。

- 出典: <https://gist.github.com/k16shikano/fd287c3133457c4fd8f5601d34aa817d>
- ライセンス: Unlicense（パブリックドメイン。著者本人が公開 gist 全体に Unlicense を明示。転記・改変・再配布いずれも条件なしで許諾。出典 URL の併記は任意だが慣行として残す）

## 適用範囲

- **発火する**: 日本語の PR 本文・docs・README・設計メモ・記事・解説文・コミットメッセージを書く／推敲する／リライトするとき。
- **発火しない**: 英語のみの出力、コードのみの変更（日本語の地の文を伴わないもの）。
