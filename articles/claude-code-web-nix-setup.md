...
```
[13:25:50] Starting Nix environment setup for Claude Code Web
[13:25:50] Installing Nix (single-user mode)...
[13:25:50] Downloading Nix 2.28.3 from nixos.org...
...
[13:27:02] Home Manager switch completed
[13:27:02] Setup complete in 72s (commit: e8ddf7f...)
```

約 **72秒** で Nix インストールから Home Manager 適用まで完了しました。

### 確認結果

| チェック項目 | コマンド | 結果 |
|-------------|---------|------|
| Nix バージョン | `nix --version` | `nix (Nix) 2.28.3` |
| fd の PATH | `which fd` | `/root/.nix-profile/bin/fd` |
| ripgrep の PATH | `which rg` | `/root/.nix-profile/bin/rg` |
| 冪等性マーカー | `cat ~/.local/state/nix-web-setup-done` | `e8ddf7f5...`（commit hash） |
...
初回セットアップでは Nix のダウンロード・インストールと nixpkgs の fetch が発生するため、十分なタイムアウトが必要です。実測で約72秒でしたが、ネットワーク状況により変動するため **300秒（5分）** を推奨します。
...
SessionStart hook → `CLAUDE_CODE_REMOTE` ガード → Nix インストール → Home Manager switch → `CLAUDE_ENV_FILE` による PATH 引き継ぎ、という流れで、約72秒でいつもの開発環境が Claude Code Web 上に再現されます。
